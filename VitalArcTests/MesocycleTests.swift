...
class MockMesocycleRepository: MesocycleRepository {
    ...
    func getMesocycle(id: UUID) async throws -> Mesocycle? {
        return mesocycles.first { $0.id == id }
    }

    func getActiveMesocycle() async throws -> Mesocycle? {
        return mesocycles.first { $0.status == .active }
    }
    ...
    func deleteMesocycle(id: UUID) async throws {
        mesocycles.removeAll { $0.id == id }
    }

    func activateMesocycle(id: UUID) async throws {
        for i in 0..<mesocycles.count {
            if mesocycles[i].id == id {
                mesocycles[i].status = .active
            } else if mesocycles[i].status == .active {
                mesocycles[i].status = .planned
            }
        }
    }

    func completeMesocycle(id: UUID) async throws {
        if let index = mesocycles.firstIndex(where: { $0.id == id }) {
            mesocycles[index].status = .completed
        }
    }

    func getMesocyclesByStatus(_ status: MesocycleStatus) async throws -> [Mesocycle] {
        return mesocycles.filter { $0.status == status }
    }

    func getMesocycleForDate(_ date: Date) async throws -> Mesocycle? {
        return mesocycles.first { $0.startDate <= date && $0.endDate >= date }
    }
}
